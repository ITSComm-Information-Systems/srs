...
def create_midesktop_ticket(action, instance, request, form, **kwargs):
    ...
    if action == 'Modify':
        payload = {
            "FormID": 111,              # ITS-MiDatabase Account Requests - Form
            "TypeID": 7,               # Compute Services
            "SourceID": 4,             #
            "StatusID": 77,            # Open
            "ServiceID": 64,            # ITS-MiDatabase Account Requests
            "ResponsibleGroupID": 86,  # ITS-MiDatabase
            "Title": "MiDesktop Modify Pool",
            "RequestorEmail": request.user.email,
            "Description": (
                f'MCommunity Group: {form.cleaned_data["admin_group"]} \n\n'
                f'Shortcode: {form.cleaned_data["shortcode"]} \n\n'
                f'Pool Display Name: {form.cleaned_data["account_id"]} \n\n'
                f'Pool Size: {form.cleaned_data["pool_maximum"]} \n\n'
                f'Pool entitlements: {form.cleaned_data["ad_access_groups"]} \n\n'
                f'Additional Details: {form.cleaned_data["additional_details"]} \n\n'
                            ),
        }
    ...
    if action == 'Delete':
        payload = {
            "FormID": 112,              # ITS-MiDatabase Account Requests - Form
            "TypeID": 7,               # Compute Services
            "SourceID": 4,             #
            "StatusID": 77,            # Open
            "ServiceID": 65,            # ITS-MiDatabase Account Requests
            "ResponsibleGroupID": 86,  # ITS-MiDatabase
            "Title": "Delete MiDesktop Pool",
            "RequestorEmail": request.user.email,
            "Description": (
                f'Pool Name: {instance.account_id} \n\n'
            )
            }
    
    if action == 'ModifyImage':
        payload = {
            "FormID": 109,              # ITS-MiDatabase Account Requests - Form
            "TypeID": 7,               # Compute Services
            "SourceID": 4,             #
            "StatusID": 77,            # Open
            "ServiceID": 61,            # ITS-MiDatabase Account Requests
            "ResponsibleGroupID": 86,  # ITS-MiDatabase
            "Title": "Modify MiDesktop Base Image",
            "RequestorEmail": request.user.email,
            "Description": (
                f'Image Name: {instance.account_id} \n\n'
                f'CPU: {instance.cpu} \n\n'
                f'Memory: {instance.memory} \n\n'
                f'Disk Space: {instance.storage} \n\n'
                f'GPU: {instance.gpu} \n\n'
                f'Base Image Total: {instance.total} \n\n'
                f'Additional Details: {form.cleaned_data["additional_details"]} \n\n'
            )
            }
    ...
    if action == 'DeleteImage':
        payload = {
            "FormID": 110,              # ITS-MiDatabase Account Requests - Form
            "TypeID": 7,               # Compute Services
            "SourceID": 4,             #
            "StatusID": 77,            # Open
            "ServiceID": 63,            # ITS-MiDatabase Account Requests
            "ResponsibleGroupID": 86,  # ITS-MiDatabase
            "Title": "Delete MiDesktop Base Image",
            "RequestorEmail": request.user.email,
            "Description": (
                f'Image Name: {instance.account_id} \n\n'
            )
