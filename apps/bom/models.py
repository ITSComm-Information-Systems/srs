...
class Estimate(BOM):
    ...
    assigned_engineer = models.ForeignKey(Technician, on_delete=models.CASCADE, limit_choices_to={'trade_id':'NA'},blank=True,null=True)
    ...
    def get_detail(self):
        ...
        self.part_list = Material.objects.filter(material_location__estimate=self).order_by('item').values('item','item__code','item__name','item__manufacturer_part_number','item__price').annotate(Sum('quantity'))
